...
eai_code::eai_code(int val, const std::string& context):error_c(val, eai_category(), context) {}
eai_code::eai_code(gaicb* req, const std::string& context):error_c(gai_error(req), eai_category(), context) {}
...
avahi_code::avahi_code(int val, const std::string& context):error_c(val, avahi_category(), context) {}
...
regex_code::regex_code(int val, const std::string& context):error_c(val, regex_category(), context) {}


auto error_handler::on_error(error_c& ec, const std::string& context) -> bool {
    if (!ec) return false;
...
auto error_handler::on_error(int ret, const std::string& context) -> bool {
    if (!ret) return false;
    errno_c ec;
    if (!context.empty()) ec.add_context(context);
    if (_on_error) { _on_error(ec);
...
auto error_c::context() const -> std::string { return _context; }

auto operator<<(std::ostream& out, const error_c& ec ) -> std::ostream& {
    out<<ec.context()<<": "<<ec.message();
    return out;
}
